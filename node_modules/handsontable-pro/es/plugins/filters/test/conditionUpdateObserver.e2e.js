describe('ConditionUpdateObserver', function () {
  ...
  function getConditionUpdateObserver() {
    return handsontable({
      filters: true
    }).getPlugin('filters').conditionUpdateObserver;
  }
  ...
  it('should fire `update` hook on every modified condition', function () {
    ...
    conditionObserver.conditionCollection.addCondition(0, {
      name: 'gt',
      args: [2]
    });
    conditionObserver.conditionCollection.removeConditions(2);
    conditionObserver.conditionCollection.addCondition(1, {
      name: 'contains',
      args: ['b']
    });
    conditionObserver.conditionCollection.addCondition(2, {
      name: 'begins_with',
      args: ['c']
    });
    conditionObserver.conditionCollection.addCondition(2, {
    ...
    conditionObserver.conditionCollection.clean(); // add 'gt'
    ...
    expect(updateSpy.calls.argsFor(0)[0].editedConditionStack.conditions[0].args).toEqual([2]); // remove

    expect(updateSpy.calls.argsFor(1)[0].editedConditionStack.column).toBe(2);
    expect(updateSpy.calls.argsFor(1)[0].editedConditionStack.conditions.length).toBe(0); // add 'contains'
    ...
    expect(updateSpy.calls.argsFor(2)[0].editedConditionStack.conditions[0].args).toEqual(['b']); // add 'begins_with'
    ...
    expect(updateSpy.calls.argsFor(3)[0].editedConditionStack.conditions[0].args).toEqual(['c']); // add 'ends_with'
    ...
    expect(updateSpy.calls.argsFor(4)[0].editedConditionStack.conditions[1].args).toEqual(['d']); // clean
  ...
  describe('groupChanges', function () {
    it('should fire `update` hook only once on `flush` method call when groupChanges is enabled', function () {
      ...
      conditionObserver.flush(); // add 'gt'

      expect(updateSpy.calls.argsFor(0)[0].editedConditionStack.column).toBe(0);
      expect(updateSpy.calls.argsFor(0)[0].editedConditionStack.conditions.length).toBe(1);
      expect(updateSpy.calls.argsFor(0)[0].editedConditionStack.conditions[0].name).toBe('gt');
      expect(updateSpy.calls.argsFor(0)[0].editedConditionStack.conditions[0].args).toEqual([2]); // add 'begins_with' and 'ends_with'
      ...
      expect(updateSpy.calls.argsFor(1)[0].editedConditionStack.conditions[1].args).toEqual(['d']); // add 'contains'
