...
describe('MultiColumnSorting', function () {
  ...
  it('should return sorting state with visual column index under `column` key by the `getSortConfig` method', function () {
    ...
    expect(getPlugin('multiColumnSorting').getSortConfig(0)).toEqual({
      column: 0,
      sortOrder: 'asc'
    ...
    expect(getPlugin('multiColumnSorting').getSortConfig(1)).toEqual({
      column: 1,
      sortOrder: 'asc'
    });
  });
  ...
  it('should change sorting indicator state when initial column sorting was provided', function () {
    ...
    var sortedColumn = spec().$container.find('th span.columnSorting')[1];
    expect(window.getComputedStyle(sortedColumn, ':before').getPropertyValue('background-image')).toMatch(/url/);
    getPlugin('multiColumnSorting').sort(); // default
  ...
  describe('Sorting more than one column by clicks', function () {
    ...
    it('should push last clicked column to the end of sort queue', function () {
      ...
      spec().sortByClickOnColumnHeader(0);
      keyDown('ctrl'); // ASC as 2nd

      spec().sortByClickOnColumnHeader(1); // DESC as 2nd
